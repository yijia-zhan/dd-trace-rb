...
RSpec.describe Datadog::Tracer do
  ...
  def sampling_priority_rate_metric(span)
    span.get_metric(Datadog::Ext::DistributedTracing::SAMPLING_PRIORITY_RATE)
  end
    ...
    context 'when a distributed trace is propagated' do
      ...
      before(:each) do
        ...
        tracer.trace(parent_span_name) do |parent_span|
          ...
          parent_span.context.sampling_priority_rate = 0.5
      ...
      it { expect(sampling_priority_rate_metric(parent_span)).to eq(0.5) }
      ...
      # Priority sampling rate should not propagate.
      it { expect(sampling_priority_rate_metric(child_span)).to be nil }
